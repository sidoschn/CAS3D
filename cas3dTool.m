...
%     sumOfZvektorsAll = sum(imageSobelZRelev,'all');
    ...
    sumOfXvektors = sum(imageSobelXpos,'all');
    sumOfYvektors = sum(imageSobelYpos,'all');
    sumOfZvektors = sum(imageSobelZpos,'all');
    sumOfXvektorsNeg = sum(imageSobelXneg,'all');
    sumOfYvektorsNeg = sum(imageSobelYneg,'all');
    sumOfZvektorsNeg = sum(imageSobelZneg,'all');
...
%     ylabel('z-elevation(°)') 
    sp2 = subplot(2,2,3);
    imagesc(imageVectorMatrix(:,:,round(stackSliceCount/2),3));
    axis off;
